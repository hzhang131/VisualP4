...
function checkCurrentWorkspace(element = null) {
  ...
  drawflowNodes.forEach((node) => {
    ...
    if (
      index !== -1 &&
      classList[index + 1] &&
      (classList[index + 2] === "prev" || classList[index + 2] === "post")
    ) {
...
function UpdateConditionBoxLocation(zoom_level, element = null) {
  ...
  var side_by_side_divs = Array.from(candidates).filter((div) => {
    let regex = new RegExp(
      `side-by-side-div-.*?-${checkCurrentWorkspace(element)}`,
    );
...
/* Shamelessly stolen from the library :) */
/* Although I did do some modifications haha */
class Drawflow {
  ...
  click(e) {
    ...
    switch (this.ele_selected.classList[0]) {
      ...
      case "drawflow":
        ...
        if (this.connection_selected != null) {
          ...
          this.connection_selected = null;
          if (
            this.precanvas.getElementsByClassName("condition-selector").length
          ) {
            this.precanvas
    ...
    if (
      ["input", "output", "main-path"].includes(this.ele_selected.classList[0])
    ) {
      e.preventDefault();
    }
  ...
  contextmenu(e) {
    ...
    if (this.node_selected || this.connection_selected) {
      ...
      if (
        ...
        var connection_id =
          this.connection_selected.parentElement.className.baseVal;
        var element = this.connection_selected.parentElement.parentElement;
        ...
        conditionbox.style.top =
          e.clientY *
            (this.precanvas.clientHeight /
              (this.precanvas.clientHeight * this.zoom)) -
          (this.precanvas.getBoundingClientRect().y + 40) *
            (this.precanvas.clientHeight /
              (this.precanvas.clientHeight * this.zoom)) +
          "px";
        conditionbox.style.left =
          e.clientX *
            (this.precanvas.clientWidth /
              (this.precanvas.clientWidth * this.zoom)) -
          (this.precanvas.getBoundingClientRect().x - 20) *
            (this.precanvas.clientWidth /
              (this.precanvas.clientWidth * this.zoom)) +
          "px";
  ...
  async contextmenuDel() {
    ...
    /* NEW: destroy the selector menu after leaving the right click context. */
      ...
      if (elemsOut[item].querySelector(".point") === null) {
        ...
        points.forEach((item, i) => {
          if (i === 0 && points.length - 1 === 0) {
            ...
            var lineCurveSearch = createCurvature(
              line_x,
              line_y,
              x,
              y,
  ...
  removeNodeId(id) {
    ...
    if (this.module === moduleName) {
      if (
        this.container.querySelector(`#${id}`).parentElement.parentElement
          .nextElementSibling
      ) {
        this.container
          .querySelector(`#${id}`)
  ...
  removeConnection() {
    if (this.connection_selected != null) {
      ...
      let nextSibling =
        this.connection_selected.parentElement.parentElement.nextElementSibling;
      if (nextSibling) {
        nextSibling.remove();
      }
